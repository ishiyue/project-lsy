...
import com.distribution.management.system.common.result.QueryList;
...
public class BeanUtils extends org.springframework.beans.BeanUtils {
    ...
    public static <T> QueryList<T> convertPage(QueryList<?> sourcePage, Class<T> clazz) {
        if (sourcePage == null) {
            return null;
        }
        List<T> targetList = convertList(sourcePage.getList(), clazz);
        return new QueryList<T>(targetList, sourcePage.getTotal());
    }

    public static <T> QueryList<T> convertPage(IPage<?> sourcePage, Class<T> clazz) {
        if (ObjectUtils.isEmpty(sourcePage) || CollectionUtils.isEmpty(sourcePage.getRecords())) {
    ...
    public static void copyProperties(Object source, Object target) {
        ...
        if (target instanceof BeanUtil) {
            ((BeanUtil.ConversionCustomizble) target).convertOthers(source);
        }
    }


    public interface ConversionCustomizable {
        /**
